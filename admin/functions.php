...
function obtener_noticias_trans($post_por_pagina, $id_transportista, $conexion){ 
	...
	$sentencia= $conexion->prepare("SELECT SQL_CALC_FOUND_ROWS * FROM noticias_trans WHERE id_trans_not=$id_transportista ORDER BY id_noticia DESC  LIMIT $inicio , $post_por_pagina"); // desde inicio hasta post por pagina.. 
	$sentencia->execute();
